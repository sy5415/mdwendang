...
* 回顾：
  ...
  * vue设计初衷：没有dom层思路；浏览器webapi；
  * .vue文件  组件：template （结构）  script （JS）  style (样式) 在一个文件内
  * 优势：组件大量复用！
  * vue：渐进式框架（有自己一套规则）  JQuery（库）
  * git：
    * gitee：（国内）远程仓储；
      * git init 
      * 远程新建仓储  
      * 关联  git remote add origin  SSH
      * git add .
      * git commit -m "xx"
      * git push -u origin master
    * GitHub：展示个人 

  * yarn serve：内存编译，开发阶段；
  * yarn build：最终项目完成了，打包编译一次；
  * 
...
* 延伸：vue3已经没有过滤器，我们也可以自己使用函数调用，返回一个结果；所以未来如何使用，看公司，看项目；
